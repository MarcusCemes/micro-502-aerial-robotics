...
class CrazyflieInDroneDome(Supervisor):
    def __init__(self):
        ...
        init_x_landing_pad, init_y_landing_pad = random.uniform(
            3.8, 4.7
        ...
        for i in range(1, 11):
            ...
            while not find_appropriate_random_position:
                ...
                new_init_x_obs, new_init_y_obs = random.uniform(
                    0.3, 4.7
                ), random.uniform(0.3, 2.7)
                min_distance = 1000
                ...
                for point in existed_points:
                    distance = np.linalg.norm(
                        [point[0] - new_init_x_obs, point[1] - new_init_y_obs]
                    )
    ...
    def read_sensors(self):
        ...
        data["v_forward"] = vx_global * np.cos(data["yaw"]) + vy_global * np.sin(
            data["yaw"]
        )
        data["v_left"] = -vx_global * np.sin(data["yaw"]) + vy_global * np.cos(
            data["yaw"]
        )
    ...
    def step(self, control_commands, sensor_data):
        ...
        motorPower = self.PID_CF.pid(
            dt,
            control_commands,
            sensor_data["roll"],
            sensor_data["pitch"],
            sensor_data["yaw_rate"],
            sensor_data["range_down"],
            sensor_data["v_forward"],
            sensor_data["v_left"],
        )
